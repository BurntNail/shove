...
use crate::cache_control::manager::{Caching, Directive};
...
pub async fn cache () -> color_eyre::Result<()> {
    ...
    match choice {
        ...
        2 => {
            let pat = Input::with_theme(&theme).with_prompt("What should the path start with?").interact()?;
...
fn get_zeroable_directives(theme: &dyn Theme) -> color_eyre::Result<Option<NonEmptyList<Directive>>> {
    Ok(if Confirm::with_theme(theme).with_prompt("Would you like any directives?").interact()? {
...
fn get_nonempty_directives (theme: &dyn Theme) -> color_eyre::Result<NonEmptyList<Directive>> {
    let number_of_directives: NonZeroUsize = Input::with_theme(theme).with_prompt("How many directives (must be >0)?").interact()?;
    let number_of_directives: usize = number_of_directives.into();

    let directives = (0..number_of_directives).map(|_| Directive::get_from_stdin(theme)).collect::<Result<_, _>>()?;
