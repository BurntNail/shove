...
use sha2::digest::{Key, KeyInit, Mac, Update};
use sha2::Sha256;
...
fn hmac (key: &Key<HmacSha256>, content: &[u8]) -> Vec<u8> {
...
impl Auth {
    ...
    pub fn protect (&mut self, pattern: String, username: String, password: String) -> color_eyre::Result<()> {
        fn Hi (key: &str, salt: &str, i: u32) -> Vec<u8> {
            let key = Key::from_slice(key.as_bytes());
            let mut prev = hmac(&key, format!("{salt}1").as_bytes());
            let mut all: Vec<Vec<u8>> = vec![prev.clone()];
            for _ in 1..i {
                let next = hmac(&key, &prev);
                all.push(next.clone());
                prev = next;
            }

            all.into_iter().reduce(|acc, item| acc.into_iter().zip(item.into_iter()).map(|(a, b)| a.bitxor(b)).collect()).unwrap()
        }
