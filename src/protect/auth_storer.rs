...
impl AuthStorer {
    ...
    pub fn get_users (&self) -> Vec<(Uuid, String)> {
        self.users.clone().into_iter().map(|(uuid, uap)| (uuid, uap.username)).collect()
    }
    ...
    pub fn add_user (&mut self, username: String, password: impl AsRef<[u8]>) -> color_eyre::Result<Uuid> {
        let mut salt = [0; 32];
        getrandom(&mut salt)?;
        ...
        self.users.insert(uuid, UsernameAndPassword {
    ...
    pub fn protect(
        ...
        *self.realms.entry(pattern)
            .or_default() = uuids;
    }

    pub fn protect_additional(
        &mut self,
        pattern: Realm,
        uuids: Vec<Uuid>,
    ) {
    ...
    pub fn find_users_with_access (&self, path: &str) -> Option<HashMap<String, String>> {
        let uuids = self.realms
            .iter()
            .find(|(pattern, _)| pattern.matches(path))
            .map(|(_, uap)| uap.clone())?;

        Some(uuids.into_iter().filter_map(|uuid| self.users.get(&uuid)).cloned().map(|uap| (uap.username, uap.stored_key)).collect())
    }
}